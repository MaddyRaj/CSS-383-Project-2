...
similarity_file = open("C:\\Users\\maddy\\OneDrive\\UW Classes\\Sophmore Year\\Quarter 3\\CSS 383\\Project 2\\The_CDS_similarity_results.csv", "w")
...
def do_BLAST_calculation(result_name):
  # run through all 13 CDS for both species and compare each CDS for each species through BLAST and get comparison percentage for those sequences. 
  ...
  # open the txt file, since we have 12 lines so 12 percentages to account for, so 
  # we go through each line in the file, and save each percentage in each line to
  # the variable above for calculating average similarity. Convert percentage to a 
  # float since it was a string when in the .txt.
  file_to_parse = open(result_name, "r+") 
  for line in file_to_parse:
    line_arr = line.strip().split("\t")
    average_similarity += float(line_arr[2])

  # divide by 12 since there are 12 total percentages and we want the average of them
  average_similarity = (average_similarity / 12)

  # get the names except the .txt since we want to know which species were compared
  two_species_names = result_name.replace(".txt", "")

  # sends the average similarity percentage to a .csv file so that we can only see the percentage similarities for all the different types of species combinations compared.
  similarity_file.write(two_species_names + " , " + str(average_similarity) + "\n")
...
def main():
  ...
  for j in range(0,17):           
    for k in range(0,17):
      ...
      # calculate the 13 CDS values for the first and second species by calling the method
      # that does all the calculate to save the sequences. 
      calculate_CDS_vals(annot_path + annotFiles[j], j, species_one_CDS)
      calculate_CDS_vals(annot_path + annotFiles[k], k, species_two_CDS)

      # save results to the specific name that has first and second species' names
      ...
      # call the method that invokes BLAST and gets percentages
